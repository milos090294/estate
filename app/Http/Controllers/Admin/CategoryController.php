...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(isset($image)){
            ...
            $slider = \Image::make($image)->resize(1600, 480)->save(public_path('storage/category/slider/'.$imagename));
            
            if(!\Storage::disk('public')->exists('category/thumb')){
                \Storage::disk('public')->makeDirectory('category/thumb');
            }
            $thumb = \Image::make($image)->resize(500, 330)->save(public_path('storage/category/thumb/'.$imagename));
